...
render.property = function(property, opts){
  opts = opts || {};
  let values = [];
  Array.prototype.forEach.call(property.items, function(item, i){
...
render.rule = function(rule, opts){
  ...
  if (rule.type === 'selector') {
    ...
    let block = '{\n';
    console.log(rule.items + (typeof rule.items));
    Array.prototype.forEach.call(rule.items, (prop, i) => {
      if (!(prop instanceof Property) && !opts.weak)
        throw new Error('selector rules may only contain properties');

      block += (opts.indent || '  ') + render.property(prop) + (i !== rule.items.length-1 ? '\n' : '');
    });
    block += '\n}';
...
let selector = new Rule('.test', [
  new Property('foo', new Value(1, 'px')),
  new Property('bar', [
    new Value(1, 'px'),
    new Value(2, 'em'),
  ])
...
console.log(render.rule(selector));
