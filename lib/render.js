...
function render(tree, opts){
  ...
  if (tree instanceof Rule) return render.rule(tree);
  if (tree instanceof Property) return render.property(tree);
  if (tree instanceof Value) return render.value(tree);
...
render.rule = function(rule, opts){
  ...
  if (rule.type === 'selector') {
    ...
    loop.call(rule.items, (prop, i) => {
      ...
      block += render.property(prop, opts) + (i !== rule.items.length-1 ? '\n' : '');
    });
    block += '\n' + '}';
    ...
    return rule.name + ' ' + expression.join(' ') + ' {\n' + block.join('\n') + '\n}';
  }
};
