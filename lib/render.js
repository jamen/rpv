...
render.rule = function(rule, opts){
  if (rule.type === 'selector') {
    ...
    loop.call(rule.items, (prop, i) => {
      ...
      block += (opts.indent ? '  '.repeat(opts.level) : '') + render(prop, opts) + (i !== rule.items.length-1 ? ';' + (opts.indent ? '\n' : ' ') : '');
    });
    opts.level--;
    block += (opts.indent ? '\n' : '') + (opts.indent ? '  '.repeat(opts.level) : '') +  '}';

    return (opts.indent ? '  '.repeat(opts.level) : '') + selector + ' ' + block;
  }
